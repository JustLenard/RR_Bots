...
chapter_links = [
    "https://www.royalroad.com/fiction/42367/12-miles-below/chapter/1058811/book-4-chapter-24-tried",
    "https://www.royalroad.com/fiction/16946/azarinth-healer/chapter/1057234/chapter-907-fissure",
]
...
# chapter_links = [
#     "https://www.royalroad.com/fiction/8694/a-story-in-black-and-white/chapter/581595/arc-1-chapter-5"
# ]
...
def my_comment_exists(comments_soup):
    ...
    for comment_container in all_comment_divs:
        ...
        if my_profile_link != None:
            # print(my_profile_link)
            # print(comment_container.find("p", string=my_comment))
            if comment_container.find("p", string=my_comment) != None:
                return True

    return False
    # print(comment_container)
    # f = comment_container.find("a", string=my_user_name)
...
def should_leave_comment(link, comment_page_number):
    ...
    if i_am_blocked(comments_soup):
        return False

    commet_exists = my_comment_exists(comments_soup)
    # if my_comment_exists(comments_soup):
    print("commet_exists", commet_exists)
    value = True
    if commet_exists:
...
def i_am_blocked(comments_soup):
    f = comments_soup.find("div", string=" The author has blocked you from commenting ")
    print(f)
    return True
...
def leave_comment():
    ...
    button = driver.find_element(
        By.CSS_SELECTOR, ".btn.btn-primary.btn-sm"
    ).get_attribute("outerHTML")
    button_soup = BeautifulSoup(button, "html.parser")
    print("button_soup", button_soup)
...
for link in chapter_links:
    ...
    # print(should_leave_comment(link, 1))

    # if should_leave_comment(link, 1):
    fuck = should_leave_comment(link, 1)
    print("fuck", fuck)
